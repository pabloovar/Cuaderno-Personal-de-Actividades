...
type
  ...
  TResiduoPDB = record
    NumRes: integer;
    Subunidad: char;
    ID3: string;             // denominación de 3 letras
    ID1: char;               //denominación de 1 letra
    Atm1, AtmN: integer;     //primer y último átomo del residuo, apuntan al número de ficha
  ...
  TPDB = record                //proteina PDB
    ...
    NumFichas, NumResiduos: integer;        //como a veces no ponen todos los átomos vamos a contar nosotros los átomos que haya
...
function plotXY(datos: TTablaDatos; img: TCanvas; OX: integer = 0;
         ...
         clpluma: Tcolor = clgreen; clrelleno: TColor=clgreen; clFondo: Tcolor =clblack;
...
//Act 2
function DetectarTipo(texto: TStrings): String;
function ObSecuencia(texto: TStrings; tipo: String): String;
...
{En este caso tenemos que crear una función que nos haga el proceso inverso a cargarPDB y nos genere una línea atom.
Lo primero que hacemos es  obtener las cadenas del número de átomo, número de residuo y las coordenadas con el
formato de tres decimales que nos interesa. Adicionalmente se hacen dos funciones que nos permiten indentar el texto que
nos intere en una longitud máxima, tenemos una para la izquierda y otra para la derecha. Estas están hechas a partir de un
código publicado en el siguiente enlace: https://webprogramacion.com/156/pascal/alinear-cadena-por-la-derecha.aspx
modificado para que cumpla la función que nos interesa.

Una vez que todo está listo, se encuadra en la línea con los espacios adecuados. Las coordenadas tienen una longitud máxima de
...
var
  j: integer;
  pdb: TStrings;
  linea, numatm, numre, corX, corY, corZ, temp: String;
  s: string;
begin
    numatm:= inttostr(atm.Numatom);
    numre := inttostr(atm.NumRes);
    corX  := formatfloat('0.000', atm.coor.X);
    corY  := formatfloat('0.000', atm.coor.Y);
    corZ  := formatfloat('0.000', atm.coor.Z);
    ...
    linea:= 'ATOM'+ '  ' + indder(5, numatm.Length, numatm) + '  ' + indizq(3,atm.ID.length, atm.ID) +
            ...
            '     ' + indder(7, corX.Length, corX)+ ' ' +  indder(7, corY.length, corY) + ' ' +
            indder(7, corZ.Length, corZ) + '  1.00 ' + indder(5, temp.length, temp);
    result:= linea;
  end;
...
function IndDer(maxlong, long: integer; var s: string): string;
//maxlong es la longitud maxima debe tener la cadena
//long es la longitud real
//s es el string que queremos indentar
const
blanco=' ';
var
j:integer;
begin
   if maxlong <> long then
      for j:=1 to maxlong-long do insert(blanco,s,j);
   result:= s;
end;
...
//Para esta nos vamos a valer de dos funciones, detectartipo y ObSecuencia (Obtener). En la primera nos valdremos de las características tipicas de cada
//archivo de bases de datos, con el objetivo de detectarlo y devolver un string que haga referencia al nombre de la base de datos en cuestión
//Este tipo será requerido en la segunda función, ya que en función del tipo que tengamos la secuencia de una letra se deberá de obtener de una manera
//diferente. En el caso del tipo PDB, tenemos desarrollada la función CargarPDB, la cual usaremos para obtener la secuencia directamente

function DetectarTipo(texto: TStrings): String;   //Detección del tipo de archivo de proteinas (ACTI 4)
var
tipo: String;
begin

   if (copy(texto[0],0,8)= 'HEADER  ') then tipo:= 'PDB';
   if (copy(texto[0],0,2)='ID') and (copy(texto[1],0,2)='XX') then tipo:= 'EMBL';
   if (copy(texto[0],0,2)='ID') and (copy(texto[1],0,2)='AC') then tipo:= 'UNIPROT';
   if (copy(texto[0],0,5)='LOCUS') then tipo:= 'GENBANK';
   result:= tipo;
end;
...
de un OpenDialog para facilitar su uso. Pide una Tescala y devuelve un string
...
//ALINEARZ
//Función que permite alinear los puntos al ejeZ. Como entrada pide un TPuntos y
//como salida devuelve el Tpuntos transformado
function alinear_ejeZ(puntos: Tpuntos): Tpuntos;
var
  salida: TPuntos;
...
//GIROOZ
{Función que nos permite generar el giro sobre el eje OZ, se crean tres diferentes
...
//GIROOY
{Función que nos permite generar el giro sobre el eje OY, se crean tres diferentes
para poder transformar un punto aislado, un conjunto específico o toda una proteína}
function GiroOY (rad: real; v: TPunto): TPunto;
var
  seno, coseno: real;
...
para poder transformar un punto aislado, un conjunto específico o toda una proteína}
function GiroOX (rad: real; v: TPunto): TPunto;            //rad=angulo en radianes
var
  seno, coseno: real;
begin
  seno:= sin(rad);
...
//TRASLACION
{Función que nos permite mover un punto la distancia que nos interesa. Se introduce
...
//PLOTXY
{Permite representar un TTablaDatos en un TCanvas. Además nos permite seleccionar
muchos parámetros importantes tales como el color de pluma, el eje Y, el X, el color
...
//MODULOV
{Obtiene el módulo de un vector TPunto}
function moduloV(A:TPunto):real;
begin
...
entre cuatro puntos. Se realiza la corrección de la IUPAC}
function torsion(A, B, C, D: Tpunto): real;
var
...
//AA3to1
{Función que nos permite obtener el identificador en código de 1 letra a partir del
de 3. Para ello hace uso de la constante AA, donde están todos los equivalentes}
...
//CARGARPDB OVERLOAD
{La función más importante de biotools. A partir de las líneas de un memo
(TStrings) nos devuelve un TPDB con todas sus características en una sola vuelta}
function CargarPDB(texto: TStrings): TPDB; overload;
var
...
begin
  ...
  for j:=1 to p.NumSubunidades do with p.sub[j] do  //pa no tener que ponerlo tol rato es PELIGROSO
  begin
